...
- 对接 `tonis2/lsp`，支持补全、跳转定义/声明、查找引用、重命名、hover/类型
  显示、签名帮助、文档符号、文档高亮、语义 tokens、折叠范围和诊断。
- 常用声明和控制流的 C3 snippets。
- 可通过 Zed settings 和 `c3fmt` 配置外部格式化。

查找引用取决于上游 `tonis2/lsp` 是否支持。本扩展不会用纯文本搜索伪造语义引用。

## 语言服务器

扩展会优先使用 `PATH` 中已安装的 `tonis2/lsp` release binary：Windows 上是
`lsp.exe`，macOS 和 Linux 上是 `lsp`。如果找不到，会为受支持的平台下载
`tonis2/lsp` release。

诊断功能要求 `tonis2/lsp` 能找到 `c3c`，通常通过 `PATH` 或项目内的 LSP
配置。扩展启动 `tonis2/lsp` 时会把诊断 debounce 设置为 250ms，而不是上游默认的
2000ms。如果需要覆盖这个值，可以在启动 Zed 前设置
`C3_ZED_DIAGNOSTICS_DELAY_MS`。

建议：维护好 `project.json` 中的 `sources`，这样跨文件和标准库相关的补全、引用
和跳转结果会更完整。

## 格式化

`c3c` 和 `tonis2/lsp` 目前都不提供格式化功能。要在 Zed 中格式化 C3，可以安装或构建
